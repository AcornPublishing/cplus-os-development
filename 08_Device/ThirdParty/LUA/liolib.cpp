...
static FILE *getfile (int arg) {
  ...
  return (FILE *)((ishandle(f)) ? lua_getuserdata(f) : NULL);
}
...
static void gc_close (void) {
  ...
  if (f != g_stdIn && f != g_stdOut && f != g_stdErr) {
    CLOSEFILE(f);
  }
}
...
static void openwithtags (void) {
  ...
  setfile(g_stdIn, FINPUT, iotag);
  setfile(g_stdOut, FOUTPUT, iotag);
