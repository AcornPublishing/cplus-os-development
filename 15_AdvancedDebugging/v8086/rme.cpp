...
//#define WARN_UNUSED_RET _Check_return_
#else
...
static WARN_UNUSED_RET int	RME_Int_ParseModRM(tRME_State *State, uint8_t **to, uint8_t **from) WARN_UNUSED_RET;
static WARN_UNUSED_RET int	RME_Int_ParseModRMX(tRME_State *State, uint16_t **to, uint16_t **from) WARN_UNUSED_RET;
...
decode:
	...
	case 0x80:
		...
		ret = RME_Int_ParseModRM(State, nullptr, &toB);
		if(ret)	return ret;
		READ_INSTR8( pt2 );
		DEBUG_S(" 0x%x", pt2);
	...
	case 0x81:
		...
		ret = RME_Int_ParseModRMX(State, nullptr, &to.W);	// Get Register Value
		if(ret)	return ret;
		#if USE_SIZE_OVERRIDES == 1
		if(State->Decoder.bOverrideOperand) {
			READ_INSTR32( dword );
	...
	case 0xC1:
		...
		ret = RME_Int_ParseModRMX(State, nullptr, &to.W);
		if(ret)	return ret;
		READ_INSTR8( pt2 );
		DEBUG_S(" 0x%x", pt2);
		#if USE_SIZE_OVERRIDES == 1
	...
	case 0xF6:
		...
		case 4:	// MUL AX = AL * r/m8
			ERROR_S("MUL (MI) AL");
			ret = RME_Int_ParseModRM(State, nullptr, &fromB);
			if(ret)	return ret;
	...
	case 0xF7:
		...
		case 4:	// MUL AX, r/m16
			...
			ret = RME_Int_ParseModRMX(State, nullptr, &from.W);
			if(ret)	return ret;
			#if DEBUG >= 2
			DEBUG_S(" (0x%x)", *from.W);
			#endif
	...
	case 0xFE:	// Register
		...
		case 0:
			DEBUG_S("INC (R)");
			ret = RME_Int_ParseModRM(State, nullptr, &toB);	//Get Register Value
			if(ret)	return ret;
			(*toB) ++;
	...
	case CMPSB:	DEBUG_S("CMPSB");
		...
		do {
			...
			if(ret)	return ret;
			ret = RME_Int_Read8(State, State->ES, State->DI.W, &byte3);
			if(ret)	return ret;
	...
	case LES:
		...
		if(ret)	return ret;
		State->ES = *from.W;
		from.W = (uint16_t *)((intptr_t)from.W + 2);
		#if USE_SIZE_OVERRIDES == 1
		if( State->Decoder.bOverrideOperand )
			*to.D = *from.D;
		else
...
static inline int RME_Int_GetPtr(tRME_State *State, uint16_t Seg, uint16_t Ofs, void* *Ptr)
{
	...
	if(addr/RME_BLOCK_SIZE && State->Memory[addr/RME_BLOCK_SIZE] == nullptr)
	# else
	if(State->Memory[addr/RME_BLOCK_SIZE] == nullptr)
	# endif
		return RME_ERR_BADMEM;
	#endif
...
//20170922
	return RME_Int_GetPtr(State, seg, addr, (void* *)ptr);
}
